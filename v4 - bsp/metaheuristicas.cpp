...
Solucao* vns(Instancia* ins, int numVizinhancas, int maxIteracoes){
    ...
    BSP* arquivo = new BSP;
    int revisitacoes = 0;
    inserir(arquivo, melhor_sol, melhor_f, &revisitacoes);
    ...
    while(iteracao < maxIteracoes){
       // std::cout <<"chegou aqui ";
        ...
        while(k <= numVizinhancas){
            vnd(ins, numVizinhancas, sol, arquivo, &revisitacoes);
    ...
    //melhor_sol->copiarPara(sol);
    delete melhor_sol;
    std::cout << revisitacoes << " revisitacoes" << std::endl;
    prettyPrint(arquivo);
...
void vnd(Instancia* ins, int numVizinhancas, Solucao* sol, BSP* arquivo, int *revisitacoes){
    ...
    while(k <= numVizinhancas){
        ...
        firstImprovement(ins, k, sol, arquivo, revisitacoes);
        f_auxiliar = sol->valorDeFuncao(ins);
...
void firstImprovement(Instancia* ins, int movimento, Solucao* sol, BSP* arquivo, int *revisitacoes){
    ...
    while(cardinalidade < cardinalidadeMaxima && !houveMelhora){
        ...
        f_auxiliar = sol->valorDeFuncao(ins);
        inserir(arquivo, sol, f_auxiliar, revisitacoes);
        if(f_auxiliar < melhor_f){
